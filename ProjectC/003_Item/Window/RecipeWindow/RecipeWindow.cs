...
public class RecipeWindow : BaseWindow
{
    ...
    [Header("作成した料理を見る確認ウィンドウコントローラー")]
    [SerializeField]
    ...
    private async UniTask CreateFood()
    {
        ...
        try
        {
            ...
            var window = await CreateToUpdateWindow<CreateFoodWindow>(createFoodController, true, async _ =>
               {
            ...
            if (window != null)
            {
                createFlg = await window.OnSelfUpdate();
            ...
            {
                // アイテム確認ウィンドウ
                var confirmationItemController = Instantiate(m_confirmationItemWindowController);
                await CreateToUpdateWindow<ConfirmationItemWindow>(confirmationItemController, false, async _ =>
                 {
                     _.SetDescription(m_pocketType, slotData.ItemTypeID, slotData.ItemID);
                     await UniTask.CompletedTask;
                 });
                if (confirmationItemController != null) Destroy(confirmationItemController.gameObject);
