...
public partial class CharacterCore : MonoBehaviour, IDamageable
{
    ...
    // ダメージ処理
    void IDamageable.Damaged(DamageNotification _dmgData, Collider _hitCol, float _knockBackMultiplier)
    {
        ...
        if (m_characterStatus.m_hp.Value > 0)
        {
            ...
            if (_hitCol)
            {
                if (!m_isNoKnockBack && (m_characterStatus.m_knockBackDamage <= _dmgData.m_damage))
                {
                    if (_knockBackMultiplier > 0.0f)
                    {
                        ...
                        // 敵であれば索敵指示をだす
                        if (EnemyParameters && gameObject.TryGetComponent(out ArborFSM arborFSM))
                        {
