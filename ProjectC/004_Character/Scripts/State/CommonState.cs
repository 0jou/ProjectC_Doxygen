...
public partial class CharacterCore : MonoBehaviour, IDamageable
{
    ...
    public class ActionState_IdleAndMove : ActionState_Base
    {
        ...
        // 追加　上甲　効果音
        private CriAtomExPlayback m_moveSound;
        public override void OnEnter()
        {
            ...
            switch (Core.GroupNo)
            {
                case CharacterGroupNumber.player:
                    {
                        if (Core.m_inputType == InputTypes.Player) Core.PlayerParameters.StartVanishWeapon();
                        m_moveSound = SoundManager.Instance.SetLoopPlayback("Walk", true);
                        m_moveSound.Pause(true);
                    }
                    break;
            }
        }
        ...
        {
            Core.m_isRun = false;
            if (Core.m_dashEffect != null) Core.m_dashEffect.Stop();
            m_moveSound.Stop();
        }
        ...
        void PlayerUpdate(Vector3 input)
        {
            ...
            {
                m_moveSound.Resume(CriAtomEx.ResumeMode.PausedPlayback);
            }
