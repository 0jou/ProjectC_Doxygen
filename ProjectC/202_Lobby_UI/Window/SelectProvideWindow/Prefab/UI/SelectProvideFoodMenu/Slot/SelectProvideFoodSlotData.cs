...
public class SelectProvideFoodSlotData : FoodSlotData
{
    // 制作者　田内
    // 提供料理(確認)スロット
   ...
   private void Check()
    {
        ...
        if (m_recipeSelectUIController.IsSelectChangeFlg == false &&
            ManagementProvideFoodManager.instance.IsListChange == false) return;
    ...
    private void SetSlot()
    {
        ...
        // 追加できなければ
        if (ManagementProvideFoodManager.instance.IsAddList() == false) return;
        ...
        foreach (var data in ManagementProvideFoodManager.instance.ProvideFoodDataList)
        {
            ...
            if ((uint)data.FoodID == itemSlotData.ItemID) return;
        }
